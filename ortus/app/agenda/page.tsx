...
export default function Agenda() {
  ...
  useEffect(() => {
      if (!formData.clinica_id) setProfissionaisFiltrados([]);
      else {
          const filtrados = profissionais.filter((p) => p.profissionais_clinicas?.some((vinculo) => vinculo.clinica_id == formData.clinica_id));
  ...
  const renderEventContent = (eventInfo) => {
      ...
      // MAPA COMPLETO DE CORES PARA CLASSES DO TAILWIND
      // Usamos classes diretas para o Tailwind conseguir compilar
      const mapBg = {
      ...
      const textClass = 'text-white'; // Fundo escuro (500) pede texto branco
      ...
      // Visualização de Semana/Dia (Blocos)
  ...
  return (
    <div className="h-[calc(100vh-6rem)] md:h-[calc(100vh-2rem)] flex flex-col space-y-4">
      ...
      {/* MODAL (Reutilizando a estrutura) */}
      {openModal && (
