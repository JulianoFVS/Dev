...
import { User, Phone, Calendar, FileText, ChevronLeft, Activity, Upload, Paperclip, Trash2, HeartPulse, Save, Loader2, AlertTriangle } from 'lucide-react';
...
export default function Prontuario() {
  ...
  async function carregarDados() {
    ...
    // 4. Anamnese (Busca ou cria objeto vazio na memória)
  ...
  async function salvarAnamnese() {
    ...
    const dados = { ...anamnese, paciente_id: params.id };
    
    // Remove o ID se for nulo para o Supabase criar um novo
    if (!dados.id) delete dados.id;

    const { data, error } = await supabase.from('anamneses').upsert(dados).select().single();
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start relative overflow-hidden">
        ...
        {alertas.length > 0 && (
            <div className="absolute top-0 left-0 w-full bg-red-50 text-red-700 text-xs font-bold px-6 py-1 flex gap-4 justify-center md:justify-start">
                <span className="flex items-center gap-1"><AlertTriangle size={12}/> ATENÇÃO CLÍNICA:</span>
                {alertas.join(' • ')}
            </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2">
            <div className="md:col-span-1 space-y-4">
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-center mb-3">
                        <h3 className="font-bold text-slate-700 text-sm flex items-center gap-2"><Paperclip size={16}/> Anexos</h3>
                        <label className="cursor-pointer text-teal-600 hover:bg-teal-50 p-1.5 rounded transition-colors"><Upload size={16} /><input type="file" className="hidden" onChange={handleFileUpload} disabled={uploading} /></label>
                    </div>
                    {uploading && <p className="text-xs text-teal-600 animate-pulse">Enviando...</p>}
                    <div className="space-y-2">
                        {arquivos.length === 0 && <p className="text-xs text-slate-400 text-center py-2">Vazio.</p>}
            ...
            <div className="md:col-span-2 space-y-4">
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">
                    {historico.length === 0 ? <div className="p-8 text-center text-slate-400">Sem histórico.</div> : historico.map(item => (
                        <div key={item.id} className="p-4 hover:bg-slate-50 transition-colors flex gap-4">
                            <div className="text-center min-w-[50px]"><span className="block text-[10px] font-bold text-slate-400 uppercase">{new Date(item.data_hora).toLocaleString('pt-BR', {month:'short'})}</span><span className="text-lg font-bold text-slate-700">{new Date(item.data_hora).getDate()}</span></div>
                            <div className="flex-1">
                                <div className="flex justify-between"><h4 className="font-bold text-slate-800 text-sm">{item.procedimento}</h4><span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 border border-slate-200 uppercase">{item.status}</span></div>
                                {item.observacoes && <div className="mt-1 text-xs text-slate-500 bg-yellow-50 p-2 rounded border border-yellow-100">{item.observacoes}</div>}
                            </div>
                        </div>
                    ))}
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {[
                            { key: 'fuma', label: 'Fumante' },
                            { key: 'bebe', label: 'Consome Álcool' },
                            { key: 'diabetico', label: 'Diabético' },
                            { key: 'hipertenso', label: 'Hipertenso' },
                            { key: 'cardiaco', label: 'Cardíaco' },
                        ].map((item: any) => (
                            <label key={item.key} className="flex items-center gap-3 cursor-pointer p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                                <input 
                                    type="checkbox" 
                                    checked={anamnese[item.key]} 
                                    onChange={e => setAnamnese({...anamnese, [item.key]: e.target.checked})}
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Alergias</label>
                        <input value={anamnese.alergias || ''} onChange={e => setAnamnese({...anamnese, alergias: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="Ex: Dipirona, Latex..." />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Medicamentos em uso</label>
                        <input value={anamnese.medicamentos || ''} onChange={e => setAnamnese({...anamnese, medicamentos: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="Ex: Losartana..." />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Cirurgias Prévias</label>
                        <input value={anamnese.cirurgias || ''} onChange={e => setAnamnese({...anamnese, cirurgias: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="Alguma internação?" />
                    </div>
                </div>
            </div>

            <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Observações Médicas Adicionais</label>
                <textarea rows={4} value={anamnese.observacoes_medicas || ''} onChange={e => setAnamnese({...anamnese, observacoes_medicas: e.target.value})} className="w-full p-3 border border-slate-200 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="Outras informações relevantes..." />
            </div>
            ...
            <div className="mt-6 flex justify-end">
                <button 
                    ...
                    className="bg-teal-600 text-white px-6 py-2.5 rounded-lg font-bold hover:bg-teal-700 transition-all flex items-center gap-2 shadow-lg shadow-teal-200"
                >
                    {salvandoAnamnese ? <Loader2 className="animate-spin"/> : <Save size={20}/>} Salvar Ficha
                </button>
            </div>
        </div>
      )}
