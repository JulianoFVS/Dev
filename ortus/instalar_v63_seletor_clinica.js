...
import { LayoutDashboard, Users, LogOut, Calendar, Menu, X, DollarSign, Settings, Building2, Bell, Mail, User, ChevronRight, ChevronsUpDown, Check } from 'lucide-react';
...
export default function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  async function validarSessao() {
    ...
    if (session) {
        ...
        if (prof) {
            ...
            // 2. Carrega Clínicas do Profissional (V63)
            const { data: vinculos } = await supabase
                .from('profissionais_clinicas')
                .select('clinica_id, clinicas(id, nome)')
            ...
            if (vinculos && vinculos.length > 0) {
                ...
                // Tenta pegar do LocalStorage ou pega a primeira
  ...
  function trocarClinica(clinica: any) {
      ...
      window.location.reload(); // Recarrega para aplicar os filtros
  }
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans">
      <aside className="w-64 bg-white border-r border-slate-200 fixed h-full hidden md:flex flex-col z-30 shadow-sm">
        ...
        {/* SELETOR DE CLÍNICA (DESKTOP) */}
        <div className="px-4 mt-6">
            <div className="relative">
                <button onClick={() => setMenuClinicaAberto(!menuClinicaAberto)} className="w-full flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl hover:border-blue-300 transition-all group">
                    <div className="flex items-center gap-3 overflow-hidden">
                        <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center border border-slate-200 text-blue-600 shadow-sm"><Building2 size={16}/></div>
                        <div className="text-left truncate">
                            <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-0.5">Clínica Atual</p>
                            <p className="text-sm font-bold text-slate-800 truncate w-24">{clinicaAtual?.nome || 'Selecione'}</p>
                        </div>
                    </div>
                    <ChevronsUpDown size={16} className="text-slate-400 group-hover:text-blue-500"/>
                </button>
