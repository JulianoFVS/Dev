    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 fixed h-full hidden md:flex flex-col z-30 shadow-sm">
        <div className="p-6 pb-2 flex justify-center">
            <img 
                src="/logo.png" alt="Logo" className="h-16 w-auto object-contain hover:scale-105 transition-transform"
      ...
      {/* CONTEÚDO PRINCIPAL */}
      <main className="flex-1 md:ml-64 flex flex-col min-h-screen transition-all pt-16 md:pt-0"> {/* AQUI: pt-16 no mobile empurra o conteúdo para baixo da barra */}
            ...
            <div className="absolute right-0 top-0 h-full w-[280px] bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-5 border-b border-slate-100 bg-white">
                    <Link href="/perfil" onClick={() => setMenuMobileAberto(false)} className="flex items-center gap-4 group">
                        <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center border-2 border-white shadow-md overflow-hidden">
                             {perfil?.foto_url ? <img src={perfil.foto_url} className="w-full h-full object-cover"/> : <User size={24}/>}
                        </div>
                        <div className="flex-1">
                            <p className="font-bold text-slate-800 leading-tight group-hover:text-blue-700">{perfil?.nome}</p>
                            <p className="text-xs text-slate-400 font-bold uppercase">{perfil?.nivel_acesso === 'admin' ? 'Administrador' : 'Profissional'}</p>
                        </div>
                ...
                <div className="p-5 border-t border-slate-100 bg-slate-50">
                    <button onClick={handleLogout} className="flex w-full items-center justify-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 rounded-xl font-bold bg-white border border-slate-200 shadow-sm transition-all active:scale-95">
