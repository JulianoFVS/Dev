...
console.log('🛠️ Instalando V33: Correção MESTRE de Tipagem para Vercel (5 Arquivos)...');

function salvarArquivo(caminhoRelativo, conteudo) {
    const caminhoCompleto = path.join(__dirname, caminhoRelativo);
    fs.writeFileSync(caminhoCompleto, conteudo.trim());
    console.log(`✅ Corrigido e Blindado: ${caminhoRelativo}`);
}

// ======================================================
// 1. INBOX (Correção do erro setTudos)
// ======================================================
const inboxPage = `
'use client';
...
export default function Inbox() {
  // CORREÇÃO: <any[]> para evitar erro de 'never'
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-2">
        <div><h2 className="text-2xl font-black text-slate-800 tracking-tight">Central de Avisos</h2><p className="text-slate-500 text-sm">Fique por dentro do que acontece na clínica.</p></div>
        <div className="flex bg-white p-1 rounded-xl shadow-sm border border-slate-200">
            <button onClick={() => setAbaAtiva('alertas')} className={\`px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all \${abaAtiva === 'alertas' ? 'bg-blue-50 text-blue-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}\`}><Bell size={16}/> Notificações {alertas.filter((n:any) => !n.lida).length > 0 && <span className="bg-red-500 text-white text-[10px] px-1.5 rounded-full">{alertas.filter((n:any) => !n.lida).length}</span>}</button>
            <button onClick={() => setAbaAtiva('mensagens')} className={\`px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all \${abaAtiva === 'mensagens' ? 'bg-purple-50 text-purple-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}\`}><Mail size={16}/> Mensagens {mensagens.filter((n:any) => !n.lida).length > 0 && <span className="bg-red-500 text-white text-[10px] px-1.5 rounded-full">{mensagens.filter((n:any) => !n.lida).length}</span>}</button>
        </div>
      </div>
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden min-h-[400px]">
        {listaAtual.length === 0 && (<div className="h-full flex flex-col items-center justify-center py-20 text-center"><div className="bg-slate-50 p-6 rounded-full text-slate-300 mb-4">{abaAtiva === 'alertas' ? <Bell size={40}/> : <Mail size={40}/>}</div><h3 className="text-slate-800 font-bold">Tudo limpo por aqui!</h3><p className="text-slate-400 text-sm mt-1">Você não tem {abaAtiva === 'alertas' ? 'novas notificações' : 'novas mensagens'}.</p></div>)}
        <div className="divide-y divide-slate-100">
            {listaAtual.map((n:any) => (
                <div key={n.id} className={\`p-5 flex gap-4 transition-all hover:bg-slate-50 group relative \${n.lida ? 'bg-white' : 'bg-blue-50/40'}\`}>
                    ...
                    <div className="mt-1 bg-white p-2 rounded-xl shadow-sm border border-slate-100 h-fit">{getIcon(n.tipo)}</div>
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                            <h4 className={\`text-sm font-bold truncate pr-4 \${n.lida ? 'text-slate-600' : 'text-slate-800'}\`}>{n.titulo}</h4>
                            <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap bg-slate-50 px-2 py-0.5 rounded border border-slate-100">{new Date(n.created_at).toLocaleDateString('pt-BR')} às {new Date(n.created_at).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}</span>
                        </div>
                    ...
                    <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity pl-2 border-l border-slate-100">
                        {!n.lida && (<button onClick={() => marcarLida(n.id)} className="text-blue-500 hover:bg-blue-100 p-2 rounded-lg" title="Marcar como lida"><CheckSquare size={18}/></button>)}
                        <button onClick={() => excluir(n.id)} className="text-slate-300 hover:text-red-500 hover:bg-red-50 p-2 rounded-lg" title="Excluir"><Trash2 size={18}/></button>
                    </div>
                </div>
            ))}
        </div>
...
// ======================================================
// 2. AGENDA (Correção profissionais_clinicas)
// ======================================================
const agendaPage = `
'use client';
...
export default function Agenda() {
  ...
  // CORREÇÃO: Tipagem explícita para evitar erros de Build
  ...
  useEffect(() => {
      if (!formData.clinica_id) {
          ...
          // CORREÇÃO CRÍTICA: (p:any) para o TypeScript aceitar .profissionais_clinicas
  ...
  const renderEventContent = (eventInfo: any) => {
      ...
      const mapBg: any = {
          slate: 'bg-slate-500', gray: 'bg-gray-500', zinc: 'bg-zinc-500', red: 'bg-red-500', orange: 'bg-orange-500', amber: 'bg-amber-500', yellow: 'bg-yellow-500', lime: 'bg-lime-500', green: 'bg-green-500', emerald: 'bg-emerald-500', teal: 'bg-teal-500', cyan: 'bg-cyan-500', sky: 'bg-sky-500', blue: 'bg-blue-500', indigo: 'bg-indigo-500', violet: 'bg-violet-500', purple: 'bg-purple-500', fuchsia: 'bg-fuchsia-500', pink: 'bg-pink-500', rose: 'bg-rose-500'
      };
      ...
      if (viewType === 'listWeek' || viewType === 'dayGridMonth') {
         return (
             <div className="flex items-center gap-1 overflow-hidden w-full">
                ...
                <span className={\`text-xs font-medium truncate \${status === 'cancelado' ? 'line-through text-slate-400' : 'text-slate-700'}\`}>{eventInfo.timeText && <span className="mr-1 opacity-70">{eventInfo.timeText}</span>}{eventInfo.event.title}</span>
             </div>
         );
      }
  ...
  return (
    <div className="h-[calc(100vh-6rem)] md:h-[calc(100vh-2rem)] flex flex-col space-y-4">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-2"><div className="bg-blue-50 p-2 rounded-lg text-blue-600"><CalIcon size={20}/></div><div><h1 className="text-lg font-bold text-slate-800 leading-tight">Agenda</h1><div className="flex items-center gap-1 text-xs text-slate-500"><Building2 size={10}/><select value={clinicaFiltro} onChange={e => setClinicaFiltro(e.target.value)} className="bg-transparent outline-none font-bold hover:text-blue-600 cursor-pointer"><option value="todas">Todas as Clínicas</option>{clinicas.map((c:any) => <option key={c.id} value={c.id}>{c.nome}</option>)}</select></div></div></div>
...
// ======================================================
// 3. CONFIGURAÇÕES (Correção setClinicas)
// ======================================================
const configPage = `
'use client';
...
// ======================================================
// 4. FINANCEIRO (Correção e.pacientes.nome)
// ======================================================
